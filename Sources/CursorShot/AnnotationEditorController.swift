...
final class AnnotationEditorController: NSObject, NSWindowDelegate {
    ...
    private var focusLossCancelWorkItem: DispatchWorkItem?
    ...
    private func showEditor(for image: CGImage) {
        ...
        window = editorWindow
        startFocusLossMonitoring(for: editorWindow)
    ...
    private func startFocusLossMonitoring(for window: NSWindow) {
        stopFocusLossMonitoring()
        appResignObserver = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                self?.cancelAfterFocusLoss(reason: "appDeactivated")
            }
        }
        DebugLog.write("annotation focus-loss monitoring started windowNumber=\(window.windowNumber)")
    }

    private func stopFocusLossMonitoring() {
        focusLossCancelWorkItem?.cancel()
        focusLossCancelWorkItem = nil
    ...
    private func cancelAfterFocusLoss(reason: String) {
        focusLossCancelWorkItem?.cancel()
        let workItem = DispatchWorkItem { [weak self] in
            Task { @MainActor in
                ...
                DebugLog.write("annotation cancelled after focus loss reason=\(reason)")
                self.complete(.cancelled)
            }
        }
        focusLossCancelWorkItem = workItem
    ...
    func windowDidResignKey(_ notification: Notification) {
        ...
        cancelAfterFocusLoss(reason: "windowResignedKey")
    }
