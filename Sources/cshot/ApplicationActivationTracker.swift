...
final class ApplicationActivationTracker {
    ...
    private func record(pid: pid_t) {
        ...
        DebugLog.write("activation tracker record pid=\(pid) order=\(orderedPIDs)")
    }
    ...
    private func describe(_ app: NSRunningApplication?) -> String {
        ...
        return "\(app.localizedName ?? "unknown")/\(app.bundleIdentifier ?? "no-bundle")/\(app.processIdentifier)"
    }
}
